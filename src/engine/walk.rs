...
pub(super) fn run_multiplexed_walk(
    ...
    walk_tree_filtered(tree, &ld.interesting, |node| {
        ...
        for &i in indices {
            // The prefilter was already answered once for this file, above.
            // Re-asking it per node would read it as a claim about the visited
            // node's own text, which is not what a rule declares: the literal
            // that makes a rule relevant often sits in a *sibling* of the
            // anchor node (a `#[derive(…)]` before a `struct_item`, the
            // `with: key:` pair beside a `uses:` one).
            if !enabled[i] {
                continue;
            }
