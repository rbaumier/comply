...
pub const META: RuleMeta = RuleMeta {
    ...
    description:
        "Entity IDs in public API signatures must use branded types, not raw string/number.",
    remediation:
        "Introduce a branded type such as `type OrderId = string & { readonly __brand: 'OrderId' }` and use it instead of `string`/`number` for ID parameters.",
