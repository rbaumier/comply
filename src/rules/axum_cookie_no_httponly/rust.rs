//! axum-cookie-no-httponly backend.
//!
//! Flags a finalized `cookie` builder chain — `Cookie::build((name, value))`
//! terminated by `.build()` or `.finish()` — that never enables the `http_only`
//! attribute. Such a cookie is readable from JavaScript (an XSS vector).
//!
...
//!
//! `.http_only(true)` and `.http_only(<variable/expr>)` (e.g.
//! `.http_only(is_api)`) mark the attribute as handled and stay silent — the
//! developer controls it. A chain with no `.http_only(...)` at all, or with an
//! explicit `.http_only(false)`, is flagged. A builder that is never finalized
//! with `.build()`/`.finish()` (a partial builder passed around) and any
//! `.build()`/`.finish()` rooted at a type other than `Cookie` are left alone.
...
crate::ast_check! { on ["call_expression"] prefilter = ["Cookie::build"] => |node, source, ctx, diagnostics|
    ...
    if !rooted_at_cookie_build || http_only_set { return; }

    diagnostics.push(Diagnostic::at_node(
        ctx.path,
        &node,
        super::META.id,
        "This `Cookie` is built without `.http_only(true)`, so it is readable \
         from JavaScript (XSS vector). Add `.http_only(true)` to the builder chain."
            .into(),
        Severity::Error,
    ));
}
...
mod tests {
    ...
    // ── Positive: finalized builder chains missing an http_only flag ───────

    #[test]
    fn flags_build_chain_without_http_only() {
        // The exact "should flag" snippet from the issue body.
        let src = r#"fn f() { let c = Cookie::build(("sid", token)).secure(true).build(); }"#;
    ...
    #[test]
    fn flags_explicit_http_only_false() {
        let src = r#"fn f() { let c = Cookie::build(("sid", token)).http_only(false).build(); }"#;
        assert_eq!(run(src).len(), 1);
    }
    ...
    // ── Negative: protected / dynamic / non-Cookie / partial chains ────────

    #[test]
    fn allows_http_only_true() {
    ...
    #[test]
    fn allows_http_only_from_variable() {
        // The flag is controlled by config/env — the developer set it; not an FP target.
        let src = r#"fn f() { let c = Cookie::build(("sid", token)).http_only(is_api).build(); }"#;
        assert!(run(src).is_empty());
    }

    #[test]
    fn allows_http_only_from_config_expr() {
        let src = r#"fn f() { let c = Cookie::build(("sid", token)).http_only(cfg.cookie_http_only).build(); }"#;
        assert!(run(src).is_empty());
    }
    ...
    fn allows_cookie_new_without_finalizer() {
        ...
        // http_only flag is set via a later `set_http_only` statement, out of scope here.
