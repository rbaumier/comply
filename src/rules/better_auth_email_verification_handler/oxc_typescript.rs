...
impl OxcCheck for Check {
    ...
    fn run<'a>(
        ...
        if prop_key_name(&prop.key) != Some("emailVerification") {
            return;
        }
        ...
        // Check sendOnSignUp: true
        let has_send_on_signup_true = obj.properties.iter().any(|p| {
            let ObjectPropertyKind::ObjectProperty(inner) = p else {
                return false;
            };
            prop_key_name(&inner.key) == Some("sendOnSignUp") && prop_value_is_true(inner)
        });
        if !has_send_on_signup_true {
            return;
        }

        if object_has_key(obj, "sendVerificationEmail") {
            return;
        }
        ...
        diagnostics.push(Diagnostic {
            ...
            message: "`sendOnSignUp: true` requires a `sendVerificationEmail` handler.".into(),
