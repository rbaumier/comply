...
impl OxcCheck for Check {
    ...
    fn run_on_semantic<'a>(
        ...
        for imp in index.get_imports(&canon) {
            ...
            // `prisma generate` and absent in a clean checkout — expected unresolved.
            if crate::rules::path_utils::resolves_into_prisma_output(
                ctx.path,
                &imp.specifier,
                ctx.project,
            ) {
            ...
            // checkout) is expected to be unresolved at lint time. Keyed on the
            // project's actual gitignore membership, honoring nested `.gitignore`
            // files — not a fixed directory-name list.
            if ctx
                .project
                .resolves_into_gitignored_path(ctx.path, &imp.specifier)
            {
                continue;
            }
...
mod oxc_tests {
    ...
    #[test]
    fn detects_framework_build_root_specifiers_issue_5158() {
        // nuxt/eslint reproducer: `playground/eslint.config.mjs` imports
        // `./.nuxt/eslint.config.mjs`, generated by `nuxt prepare` into the
        // gitignored `.nuxt/` build directory, absent in a clean checkout.
        assert!(is_build_output_specifier("./.nuxt/eslint.config.mjs"));
        assert!(is_build_output_specifier("../.output/server/index.mjs"));
        assert!(is_build_output_specifier("./.nitro/types/nitro.d.ts"));
        assert!(is_build_output_specifier("./.next/types/app/page.ts"));
        assert!(is_build_output_specifier("./.vinxi/build/client.js"));
        assert!(is_build_output_specifier("./.svelte-kit/generated/root.js"));
        // still flagged — the dot prefix is required, so a hand-written
        // `./nuxt/` source dir (no leading dot) stays a real broken path
        assert!(!is_build_output_specifier("./nuxt/config"));
        assert!(!is_build_output_specifier("./output/result"));
    }
}
...
mod framework_generated_route_types_tests {
    ...
    #[test]
    fn no_fp_for_nuxt_build_dir_import_issue_5158() {
        // nuxt/eslint reproducer: `playground/eslint.config.mjs` imports
        // `./.nuxt/eslint.config.mjs`, generated by `nuxt prepare` into the
        // gitignored `.nuxt/` build directory and absent in a clean checkout, so
        // the import must not be flagged.
        let diags = run_in_dir(
...
mod prisma_output_tests {
    ...
    /// run the rule on the importer, and return its diagnostics. The generator
    /// `output` directory is never created — it is absent in a clean checkout.
    fn run_with_schema(
        importer_rel: &str,
        source: &str,
        schema_rel: &str,
    ...
    fn no_fp_for_custom_prisma_output_import_issue_2293() {
        // prisma/prisma reproducer: an import of the generated client from the
        // `generator { output = "./client" }` directory. That dir is created by
        // `prisma generate`, gitignored, and absent in a clean checkout, so the
        // import must not be flagged.
    ...
    fn still_flags_missing_non_prisma_import_with_schema_present() {
        // Negative space: with the Prisma signal present, an import outside the
        // generator output dir is a genuine broken path and must still fire.
    ...
    fn still_flags_prisma_shaped_import_without_schema() {
        // No `schema.prisma` = no Prisma signal: a `./client/edge` import is just
        // a missing path and must still fire. The exemption is gated on a real
        // generator `output` declaration, not the path shape.
        let source = "import { PrismaClient } from './client/edge';";
        let diags = run_with_schema(
            "packages/bundle-size/da-workers-pg/index.js",
            source,
