...
impl TextCheck for Check {
    fn check(&self, ctx: &CheckCtx) -> Vec<Diagnostic> {
        ...
        for block in scan_blocks(ctx.source) {
            let tags = block.tags();
            // Only police blocks that actually declare properties — i.e.
            // a typedef/interface block.
 ...
 */
"#;
        // `@param` duplication is the scope of another rule; this rule
        // only cares about `@property`.
        assert!(run(src).is_empty());
    }
