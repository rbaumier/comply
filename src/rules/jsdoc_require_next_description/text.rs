//! jsdoc-require-next-description backend — `@next` must have a description.
    ...
    fn check(&self, ctx: &CheckCtx) -> Vec<Diagnostic> {
        ...
        for (block_start, block) in extract_jsdoc_blocks(ctx.source) {
            for (line_offset, line) in block.lines().enumerate() {
                ...
                if let Some(after) = trimmed.strip_prefix("@next") {
                    ...
                    if desc.is_empty() {
                        diagnostics.push(Diagnostic {
                            ...
                            message: "`@next` tag is missing a description.".into(),
...
"#;
    ...
    fn allows_tag_with_description() {
        let src = r#"
/**
 * @next value - a detailed description here
 */
"#;
