//! jsdoc-require-rejects backend — `@rejects` needed when promise rejects.
...
impl TextCheck for Check {
    fn check(&self, ctx: &CheckCtx) -> Vec<Diagnostic> {
        ...
        for (block_start, block) in &blocks {
            ...
            let has_rejects = tags.iter().any(|(_, t, _)| t == "rejects");
            if has_rejects {
                continue;
            }

            // Check following code for reject patterns
            ...
            if code_after.contains("reject(") || code_after.contains("Promise.reject") {
...
"#;
    ...
    #[test]
    fn allows_documented_rejects() {
        let src = r#"
/**
 * Fetch data.
 * @rejects {Error} on failure
 */
async function fetchData() {
    return Promise.reject(new Error("fail"));
}
