...
fn has_return_expr(body: &str) -> bool {
    ...
    while let Some(pos) = body[search_from..].find("return") {
        ...
        // Ensure "return" is not part of a larger identifier.
        if abs > 0 {
        ...
        // Check the char right after "return".
        match after_return.chars().next() {
            Some(c) if c.is_ascii_alphanumeric() || c == '_' || c == '$' => {
                // Part of a longer identifier like "returnValue".
                search_from = abs + 6;
                continue;
            }
            _ => {}
...
fn has_returns_tag(lines: &[&str], start: usize, end: usize) -> bool {
...
impl TextCheck for Check {
    fn check(&self, ctx: &CheckCtx) -> Vec<Diagnostic> {
        ...
        while i < lines.len() {
            ...
            if !has_returns_tag(&lines, block_start, block_end)
                ...
                && has_return_value(&lines, fn_line)
            {
                diagnostics.push(Diagnostic {
                    ...
                    message: "Function returns a value but JSDoc is missing `@returns`. Document what the function returns.".into(),
