//! jsdoc-require-returns-description backend — `@returns` must have a description.
    ...
    fn check(&self, ctx: &CheckCtx) -> Vec<Diagnostic> {
        ...
        for (block_start, block) in extract_jsdoc_blocks(ctx.source) {
            for (line_offset, line) in block.lines().enumerate() {
                ...
                let tag = if trimmed.starts_with("@returns") {
                    Some("@returns")
                ...
                if let Some(tag_str) = tag {
                    let after = trimmed[tag_str.len()..].trim_start();
