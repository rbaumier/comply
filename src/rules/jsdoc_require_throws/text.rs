//! jsdoc-require-throws backend — `@throws` needed when function throws.
//!
//! Also covers Rust `panic!` in doc comments.
...
impl TextCheck for Check {
    fn check(&self, ctx: &CheckCtx) -> Vec<Diagnostic> {
        ...
        for (block_start, block) in &blocks {
            ...
            let has_throws = tags.iter().any(|(_, t, _)| t == "throws" || t == "exception");
            if has_throws {
                continue;
            }

            // Check following code for throw statements
            ...
            let throws = code_after.contains("throw ") || code_after.contains("throw(");
            // Also check Rust panics
            let panics = code_after.contains("panic!(") || code_after.contains("unwrap()") || code_after.contains("expect(");
