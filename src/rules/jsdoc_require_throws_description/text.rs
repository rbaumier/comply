//! jsdoc-require-throws-description backend — `@throws` must have a description.
    ...
    fn check(&self, ctx: &CheckCtx) -> Vec<Diagnostic> {
        ...
        for (block_start, block) in extract_jsdoc_blocks(ctx.source) {
            for (line_offset, line) in block.lines().enumerate() {
                ...
                if let Some(after) = trimmed.strip_prefix("@throws") {
...
"#;
    ...
    fn allows_tag_with_description() {
        let src = r#"
/**
 * @throws {Error} - a detailed description here
 */
"#;
