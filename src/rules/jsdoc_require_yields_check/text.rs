//! jsdoc-require-yields-check backend — `@yields` present when `yield` used.
...
impl TextCheck for Check {
    fn check(&self, ctx: &CheckCtx) -> Vec<Diagnostic> {
        ...
        for (block_start, block) in &blocks {
            ...
            // Check if the following code contains `yield`
            ...
            let has_yield_stmt = code_after.contains("yield ");
            if has_yield_stmt && !has_yields {
...
"#;
    ...
    #[test]
    fn allows_yield_with_tag() {
        let src = r#"
/**
 * Numbers.
 * @yields {number} values
 */
function* gen() {
    yield 1;
}
