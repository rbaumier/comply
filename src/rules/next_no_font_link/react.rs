...
mod tests {
    ...
    fn flags_googleapis_font_link() {
        let diags = run(r#"function App() { return <link href="https://fonts.googleapis.com/css2?family=Inter" rel="stylesheet" />; }"#);
        assert_eq!(diags.len(), 1);
    }

    #[test]
    fn flags_gstatic_font_link() {
        let diags = run(r#"function App() { return <link href="https://fonts.gstatic.com/s/inter/v12/abc.woff2" rel="preload" as="font" />; }"#);
        assert_eq!(diags.len(), 1);
    }

    #[test]
    fn flags_googleapis_in_opening_tag() {
        let diags = run(r#"function App() { return <link href="https://fonts.googleapis.com/css?family=Roboto" rel="stylesheet"></link>; }"#);
        assert_eq!(diags.len(), 1);
    }

    #[test]
    fn allows_non_font_link() {
        assert!(run(r#"function App() { return <link href="/styles.css" rel="stylesheet" />; }"#).is_empty());
    }
