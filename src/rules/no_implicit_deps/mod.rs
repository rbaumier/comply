...
/// True if `spec` begins with a `~`-rooted or `@/` path-alias prefix.
///
/// Both shapes are structurally impossible npm package names, so a bare
...
///   - `~…`: npm package names cannot begin with `~`, so any `~`-rooted
///     specifier is an alias — `~/…` (app root), `~~/…` (Nuxt project root),
///     etc.
///   - `@/…`: an npm scoped name is `@scope/name` with a non-empty scope; `@/`
///     has an empty scope, so it can never name a package.
///
/// A legitimate scoped package (`@scope/pkg`, non-empty scope) is not exempted.
/// Whether the alias resolves on disk is `import-no-unresolved`'s concern, not
/// this dependency-declaration rule's.
