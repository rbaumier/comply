//! no-weak-cipher AST backend — weak ciphers (DES, RC4, RC2, Blowfish).
...
mod tests {
    ...
    fn flags_createcipheriv_des() {
        assert_eq!(
            run_on("const c = crypto.createCipheriv('des-ecb', key, iv);").len(),
            1
        );
    }
    ...
    fn flags_algorithm_rc4() {
        assert_eq!(run_on("algorithm: 'rc4'").len(), 1);
    }

    #[test]
    fn flags_cipher_blowfish() {
        assert_eq!(run_on("cipher: 'blowfish'").len(), 1);
    }

    #[test]
    fn allows_aes_256_gcm() {
        assert!(run_on("const c = crypto.createCipheriv('aes-256-gcm', key, iv);").is_empty());
    }

    #[test]
    fn ignores_des_outside_crypto_context() {
        assert!(run_on("const description = 'some des text';").is_empty());
    }
}
