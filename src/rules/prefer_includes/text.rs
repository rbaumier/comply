...
mod tests {
    ...
    fn flags_indexof_not_equal_minus_one() {
        assert_eq!(run("if (arr.indexOf(x) !== -1) {}").len(), 1);
    }
    ...
    #[test]
    fn flags_indexof_strict_equal_minus_one() {
        assert_eq!(run("if (arr.indexOf(x) === -1) {}").len(), 1);
    }

    #[test]
    fn flags_indexof_lt_zero() {
        assert_eq!(run("if (arr.indexOf(x) < 0) {}").len(), 1);
    }

    #[test]
    fn flags_lastindexof() {
        assert_eq!(run("if (str.lastIndexOf(c) !== -1) {}").len(), 1);
    }

    #[test]
    fn allows_includes() {
        assert!(run("if (arr.includes(x)) {}").is_empty());
    }

    #[test]
    fn allows_indexof_other_comparison() {
        assert!(run("if (arr.indexOf(x) === 2) {}").is_empty());
    }
}
