...
//! pattern can match the empty string. A pattern matches empty iff it is
//! "nullable": every term of some top-level alternative is nullable. A
//! mandatory atom (a literal/class/`.` next to a nullable quantifier, e.g.
//! `\n\s*`) makes the alternative non-nullable, so it is not flagged.
//! Matching empty is a footgun for split/replace.
//!
