...
impl OxcCheck for Check {
    ...
    fn run<'a>(
        ...
        // not the deployed app. The app author installs `helmet()` (or sets
        // HSTS) in their own bootstrap, so the library wrapper must not be
        // required to. A real app is never a library, so genuine apps stay
        // checked.
        if ctx
            .project
...
mod tests {
    ...
    #[test]
    fn flags_bare_express_app_without_hsts() {
    ...
    #[test]
    fn allows_express_app_with_explicit_hsts_header() {
        let pkg = r#"{ "name": "my-app", "private": true }"#;
    ...
    fn skips_express_in_published_library_with_publish_config() {
        ...
        // `helmet()`/HSTS in their own bootstrap, so the adapter must not flag.
        let pkg = r#"{
    ...
    fn flags_express_in_non_library_application_package() {
        ...
        // the deployed app must set HSTS, so no security false negative.
        let pkg = r#"{ "name": "my-app", "private": true }"#;
        let src = "const app = express();\n";
        assert_eq!(run_with_pkg(pkg, src).len(), 1);
    }
