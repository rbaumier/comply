//! shadcn-sheet-requires-title OXC backend — each `<SheetContent>` must
//! contain a `<SheetTitle>` descendant.
        ...
        for node in semantic.nodes().iter() {
            ...
            if !children_have_title(&el.children) {
                ...
                diagnostics.push(Diagnostic {
                    ...
                    message: "`<SheetContent>` is missing `<SheetTitle>` — required for screen readers.".into(),
