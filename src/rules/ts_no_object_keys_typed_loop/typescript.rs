//! Heuristic detection: search for `Object.keys(<ident>)` followed shortly by
//! `.forEach(` or `.map(` whose callback body indexes the same identifier
//! with the loop variable. This is text-based, so we keep the patterns
...
/// Try to extract `<ident>` from a substring starting after `Object.keys(`.
/// Returns the identifier and the byte offset where it ends.
    ...
    fn check(&self, ctx: &CheckCtx) -> Vec<Diagnostic> {
        ...
        for (idx, line) in ctx.source.lines().enumerate() {
            ...
            while let Some(rel) = line[search_from..].find("Object.keys(") {
                ...
                // After the ident we expect `).forEach(` or `).map(`.
                ...
                // Now check whether the rest of the line (and a couple
                // following lines) references `ident[` — i.e. indexing the
                // same object with a non-keyof key. Cheap proxy: see if
                // the line contains `ident[`.
