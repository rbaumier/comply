//! use-vue-valid-v-pre AST backend.
//!
//! Walks `directive_attribute` nodes. For each one whose `directive_name` is
//! `v-pre`, reports when the directive carries an argument, modifiers, or a
//! value. Only the bare `v-pre` form is accepted.

use crate::diagnostic::{Diagnostic, Severity};

/// The kind of `v-pre` violation, mapped to its diagnostic message.
enum Violation {
    Argument,
    Modifiers,
    Value,
...
impl Violation {
    fn message(&self) -> &'static str {
        match self {
            Self::Argument => "The v-pre directive must not have an argument.",
            Self::Modifiers => "The v-pre directive does not support modifiers.",
            Self::Value => "The v-pre directive must not have a value.",
        }
    }
}
...
/// Classify a `v-pre` `directive_attribute`, returning the first violation
/// found in Biome's check order (argument, modifier, value), or `None` when
/// the usage is the bare `v-pre`.
...
crate::ast_check! { on ["directive_attribute"] prefilter = ["v-pre"] => |node, source, ctx, diagnostics|
    if directive_name(node, source) != Some("v-pre") {
        return;
    }
...
mod tests {
    ...
    #[test]
    fn flags_argument() {
        let diags = run(&wrap("<div v-pre:arg></div>"));
    ...
    #[test]
    fn flags_modifier() {
        let diags = run(&wrap("<div v-pre.mod></div>"));
        assert_eq!(diags.len(), 1);
    ...
    #[test]
    fn flags_value() {
        let diags = run(&wrap("<div v-pre=\"value\"></div>"));
        assert_eq!(diags.len(), 1);
    ...
    #[test]
    fn allows_bare_v_pre() {
        assert!(run(&wrap("<div v-pre></div>")).is_empty());
    }
    ...
    #[test]
    fn allows_bare_v_pre_alongside_plain_attributes() {
        assert!(run(&wrap("<div id=\"x\" class=\"y\" v-pre></div>")).is_empty());
    }
