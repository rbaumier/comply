//! vue-valid-v-cloak AST backend.
//!
//! Walks `directive_attribute` nodes. For each one whose `directive_name` is
//! `v-cloak`, reports when the directive carries an argument, modifiers, or a
//! value. Only the bare `v-cloak` form is accepted.

use crate::diagnostic::{Diagnostic, Severity};

/// The kind of `v-cloak` violation, mapped to its diagnostic message.
enum Violation {
    Argument,
    Modifiers,
    Value,
...
impl Violation {
    fn message(&self) -> &'static str {
        match self {
            Self::Argument => "`v-cloak` cannot have an argument.",
            Self::Modifiers => "`v-cloak` cannot have modifiers.",
            Self::Value => "`v-cloak` cannot have a value.",
        }
    }
}
...
/// Classify a `v-cloak` `directive_attribute`, returning the first violation
/// found in Biome's check order (argument, modifier, value), or `None` when
/// the usage is the bare `v-cloak`.
...
crate::ast_check! { on ["directive_attribute"] prefilter = ["v-cloak"] => |node, source, ctx, diagnostics|
    if directive_name(node, source) != Some("v-cloak") {
        return;
    }
...
mod tests {
    ...
    #[test]
    fn flags_argument() {
        let diags = run(&wrap("<div v-cloak:arg></div>"));
    ...
    #[test]
    fn flags_modifier() {
        let diags = run(&wrap("<div v-cloak.mod></div>"));
        assert_eq!(diags.len(), 1);
    ...
    #[test]
    fn flags_value() {
        let diags = run(&wrap("<div v-cloak=\"value\"></div>"));
        assert_eq!(diags.len(), 1);
        assert!(diags[0].message.contains("cannot have a value"));
    }
    ...
    #[test]
    fn allows_bare_v_cloak() {
        assert!(run(&wrap("<div v-cloak></div>")).is_empty());
    }

    #[test]
    fn allows_bare_v_cloak_alongside_plain_attributes() {
        assert!(run(&wrap("<div id=\"x\" class=\"y\" v-cloak></div>")).is_empty());
    }
    ...
    #[test]
    fn ignores_v_if_with_value() {
        assert!(run(&wrap("<div v-if=\"flag\"></div>")).is_empty());
    }
}
